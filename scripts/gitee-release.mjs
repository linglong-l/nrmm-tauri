...
function defaultReleaseBody(existingBody) {
  const defaults = [
    `## nrmm-rust ${tag}`,
    '',
    '自动构建发布（Gitee Go Linux 构建 + 本地 Windows 构建）。',
    '',
    '### Windows 产物',
    '- **NSIS 安装包**: `nrmm-rust-setup-x86_64.exe` — 推荐，带安装向导和自动卸载',
    '- **MSI 安装包**: `nrmm-rust-x86_64.msi` — 企业部署/域策略安装',
    '- **便携版 (zip)**: `nrmm-rust-portable-x86_64.zip` — 解压即用，不写注册表',
    '',
    '### Linux 产物',
    '- **AppImage**: `nrmm-rust-x86_64.AppImage` — 通用 Linux 包，chmod +x 直接运行',
    '- **deb**: `nrmm-rust-x86_64.deb` — Debian/Ubuntu/Mint 系',
    '- **rpm**: `nrmm-rust-x86_64.rpm` — Fedora/RHEL/CentOS/openSUSE 系',
    '- **便携版 (tar.gz)**: `nrmm-rust-portable-x86_64.tar.gz` — 解压即用',
    '',
    '### 自更新',
    '应用内检查更新使用自定义 Gitee API 逻辑（不再依赖 Tauri 官方 updater endpoint）。',
  ].join('\n');
